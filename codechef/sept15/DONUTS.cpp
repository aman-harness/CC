...
int main(){
	...
	int long long input[30000];
		long long int t;si(t);
		while(t--){
			long long int n, m;
			si(n);si(m);
			RNG(i, m) si(input[i]);
			sort(input, input + m);
			long long int to = 0, i = 0, size = m, res = 0;
			...
			if(size == 1) {cout << to << endl; continue;}
			else cout << to + size - 1 << endl;
			// cout << to << size <<endl;
			// if(input[i] == size - 2) cout << to + input[i] << endl;
			// else{
			// 	cout << to + size - 1 << endl;
			// }
	}
