...
template<typename T> T modPow(T b, T e, T m=(ll)MOD){T ans=1;while(e){if(!(e&0x1))ans=(ans*b)%m;e>>=1;b=(b*b)%m;}return ans; }
...
int main(){
	...
	while(1){
		cin >> n;if(!(1 + n)) break;
		// long long unsigned int *dp = new long long unsigned int[n+ 2];
		// FOR(i, 0, n + 1) dp[i] = 0;
		FOR(i, 0 , n+1) {ans[i] = bigint("0");}
		// dp[0] = 1;

		ans[0] = bigint("1"); ans[1] = bigint("1"); ans[2] = bigint("2");
		// dp[1] = 1;dp[2] = 2;
		FOR(i, 3, n + 1){
			FOR(j, 0, i){
				// cout <<  i - j -1 << ' ' <<dp[ i - j -1] << " " << j << " " << dp[j] << endl;
				// dp[i] += dp[ i - j -1] * dp[j];
