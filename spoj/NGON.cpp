...
int call(int sum , int n, int * A){
    RNG(i, n +2) RNG(j, n + 1) dp[i][j] = 0;
    RNG(i, n + 1) {dp[0][i] = 0;dp[i][0] = 1;}
    FOR(i, 1, n + 1){
        FOR(j, 1, (n + 1)){
            dp[i][j] = dp[i - 1][ j ] + dp[i - 1][j - 1] * A[i] + dp[i - 1][j - 2] * ((A[i] )* (A[i] - 1))/2;
            if(!dp[i][j]) break;
            dp[i][j] %= MOD;
            // recurse(a,b) = recurse(a-1,b) + recurse(a-1,b-1)ai +recurse(a-1,b-2)ai*(ai-1)/2
        }
    }
    // 
    // RNG(i, n + 1) {RNG(j, n + 1) cout << dp[i][j] << " "; cout << endl;}
    return dp[n][n - 1];
}
...
int main(){
    ...
    while(t--){
        sum = 0;
        cin >> temp; k = temp;int A[temp + 3];
        FOR(i, 1, k + 1) {cin >> A[i]; sum += A[i];}
        // cout << sum << k << endl;   
        cout << call(sum, k, A) << endl;
    } 
return 0;
}
