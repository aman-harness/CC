...
int main(){
	...
	int t; cin >> t;
	priority_queue<int, vector<int>, greater<int> > from, to;
	while(t--){
		to = priority_queue <int, vector<int>, greater<int> >(); from = priority_queue <int, vector<int>, greater<int> >();
		int n, x, y;
		cin >> n;
		...
		while(!from.empty())
			...
			// cout << from.top() << to.top() << count << endl;
			} 
		cout << max << endl;	
 	}
return 0;
